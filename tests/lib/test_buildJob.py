...
class TestBuildJob(TestCase):
    def setUp(self):
        self.build_job = BuildJob(dict(name='Test Build Job'))

    def test_job_name_validation(self):
        self.assertRaises(ValueError, BuildJob, dict(name=None))
        self.assertRaises(ValueError, BuildJob, dict(name=''))

    def test_num_leds_validation(self):
        self.assertRaises(ValueError, BuildJob, dict(name='some', num_leds=0))
        self.assertRaises(ValueError, BuildJob, dict(name='some', num_leds=33))

    def test_offset_validation(self):
        self.assertRaises(ValueError, BuildJob, dict(name='some', num_leds=2, offset=32))
        self.assertRaises(ValueError, BuildJob, dict(name='some', num_leds=2, offset=-1))
    ...
    def test_led_addresses_for_multiple_leds(self):
        build_job = BuildJob(dict(name='Some Job', num_leds=4))
    ...
    def test_next_index_with_large_padding(self):
        build_job = BuildJob(dict(name='Some Job', num_leds=4, offset=5))
