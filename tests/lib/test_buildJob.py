...
class TestBuildJob(TestCase):
    def setUp(self):
        self.build_job = BuildJob(32, 'Test Build Job')

    def test_job_name_validation(self):
        self.assertRaises(ValueError, BuildJob, 32, None)
        self.assertRaises(ValueError, BuildJob, 32, '')

    def test_num_leds_validation(self):
        self.assertRaises(ValueError, BuildJob, 32, 'some', 0)
        self.assertRaises(ValueError, BuildJob, 32, 'some', 33)

    def test_offset_validation(self):
        self.assertRaises(ValueError, BuildJob, 32, 'some', 2, 32)
        self.assertRaises(ValueError, BuildJob, 32, 'some', 2, -1)
    ...
    def test_default_build_job(self):
        ...
        self.assertEqual(self.build_job.total_leds, 32)
    ...
    def test_led_addresses_for_multiple_leds(self):
        build_job = BuildJob(32, 'Some Job', num_leds=4)
    ...
    def test_next_index_with_large_padding(self):
        build_job = BuildJob(32, 'Some Job', num_leds=4, offset=5)
